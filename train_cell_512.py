...
parser.add_argument('--trained_model', default='weights/ssd300_v2.pth',
...
def train():
    ...
    # since conf layer's shape related to num_class, new shape must be defined here.
    mbox = [4, 4, 6, 6, 6, 6, 6, 6, 4, 4, 4, 4]
    j = 0
    for i, (k, v) in enumerate(trained_model.items()):
        if i < (length_model - 12):
            mdfmodel[k] = v
        else:
    ...
    for para in net.vgg[:29].parameters():
    ...
    epoch_size = len(dataset) // args.batch_size  # epoch: 3
    ...
    for iteration in range(args.start_iter, cfg['max_iter']):
        ...
        if iteration != 0 and iteration % 5000 == 0:
            ...
            torch.save(ssd_net.state_dict(), os.path.join(os.getcwd(), args.save_folder) +
                       args.dataset +'-' + freezy_name + '-' +
                       repr(iteration) + '-lr' + '.pth')
            # /work/scratch/zhou/MA/ssd_cell/weights/CELLconv4_x_repr(iteration).pth

    torch.save(ssd_net.state_dict(), os.path.join(os.getcwd(), args.save_folder) +
                args.dataset + '-' + freezy_name + '-' +
               repr(cell['max_iter']) + + '-lr' + '.pth')
