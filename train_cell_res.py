...
from ssd_res101 import build_ssd_res
import os
import sys
import time
import torch
...
parser.add_argument('--basenet', default = 'resnet101-5d3b4d8f.pth',
...
def train():
    ...
    ssd_net = build_ssd_res('train', cfg['min_dim_res'], cfg['num_classes'])
    net = ssd_net
    # print(net)
    ...
    if args.use_pretrained:
        ...
        res_weights = torch.load(args.save_folder + args.basenet)
        print('Loading base network...')
        # print(res_weights.keys())
        # print(net.res.state_dict().keys()) -> print dict of current network
        ssd_net.res.load_state_dict({k:v for k,v in res_weights.items()
                                     if k in net.res.state_dict()})
